...
export default function Line(p: LineProps) {
  ...
  const onLyricLayout = (event: NativeSyntheticEvent<TextLayoutEventData>, i: number) => {
    ...
    setWidths((prev) => {
      ...
      // stops onMaxWidth call, if any of the widths are not populated
      for (let j=0; j<prev.length; j++) {
        if (prev[j] === -1) return [...prev]
      }

      const max = Math.max(...prev)
      console.log(`MAX WIDTH FOUND: ${max}`)
      p.onMaxWidth(max)
      return [...prev]
    })
  }
    ...
    { phrase && phrase.lyric.trim() != "" && <View className="relative h-12 mt-6">
      <View>
        { phrase.chords.map((c, i) => { 
          ...
          const s = sum(widths, i); console.log(`chord ${i}: w = ${s}`)
          ...
          return <Text className={`absolute ${p.textSize || "text-lg"} bold ${p.secondary ? "bg-gray-700 text-gray-400" : "bg-oslyn-700 text-white" } px-1 rounded-lg`} style={{
            marginLeft: s || 0, left: 0, top: -24}} key={i}>{chord}{p.decorate && c.decorator}</Text>
        })}
      </View>

      <View>
        <Text className={`${p.textSize || "text-lg"} ${p.secondary ? "text-gray-700 dark:text-gray-400": "text-oslyn-700 dark:text-oslyn-200"} whitespace-nowrap`}>
          {phrase.lyric}
        </Text>
        ...
        { phrase.chords.map((c, i) => {
          ...
          return <Text className={`absolute h-0 opacity-0 ${p.textSize || "text-lg"}`} key={i} numberOfLines={1} onTextLayout={(e) => { onLyricLayout(e, i) } }>{lyric}</Text>
        })}
      </View>
