...
import DownloadPdf from "@/app/songs/(components)/pdf"
...
export default function SongTable(p: SongTableProps) {
  ...
  const [openDownload, setOpenDownload] = useState<number>(-1)
            ...
            {songs.map((a, i) => <tr key={i} className="bg-white border-b dark:bg-gray-900 dark:border-gray-700">
              ...
              <td className="px-6 py-4">
                <div className="flex flex-row">
                  ...
                  <button type="button" onClick={() => setOpenDownload(i)}
              ...
              <DownloadPdf song={songs[i]} open={ openDownload === i } 
                setOpen={(b: boolean) => { if (b) setOpenDownload(i); else setOpenDownload(-1) } } 
              />
            </tr>)}
        </tbody>
      </table>
